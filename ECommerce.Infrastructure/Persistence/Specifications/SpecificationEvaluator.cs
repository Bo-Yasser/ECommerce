...
public class SpecificationEvaluator
{
        ...
        ISpecification<T> specification) where T : class
    {
        ...
        foreach(var include in specification.Includes) // add parent includes
        {
            var path = GetMemberPath(include.Body);
            query = query.Include(path);
            pathMap[include] = path;
        }
        foreach(var includeInfo in specification.IncludeExpressions) // add ThenInclude with their matching parent includes 
        {
