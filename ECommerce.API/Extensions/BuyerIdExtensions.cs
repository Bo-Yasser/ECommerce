...
public static class BuyerIdExtensions
{
    ...
    public static Guid GetValidatedBuyerId(this ControllerBase controller)
    {
        if (controller.HttpContext.Items.TryGetValue("ValidatedBuyerId", out var buyerIdObj) &&
            buyerIdObj is Guid buyerId)
        {
            return buyerId;
        }

        throw new InvalidOperationException("BuyerId was not validated by the filter.");
    }
}